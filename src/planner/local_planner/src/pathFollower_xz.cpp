...
int main(int argc, char** argv)
{
  ...
  // ros::Subscriber subGoal = nh.subscribe<geometry_msgs::PoseStamped> ("/way_point", 5, goalHandler);

  //发布导航状态
  ros::Publisher pubnavState = nh.advertise<std_msgs::Bool> ("/nav_state", 5);
  std_msgs::Bool navpose_stateMsgs;
  // navpose_stateMsgs;
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      if (fabs(vehicleSpeed) < 10.0 * maxAccel / 100.0) vehicleYawRate = -stopYawRateGain * dirDiff;
      else vehicleYawRate = -yawRateGain * dirDiff;
      ...
      //zbh发布导航状态
      bool navpose_state = false;
      navpose_stateMsgs.data = navpose_state;
      pubnavState.publish(navpose_stateMsgs);
    ...
    else if(navpoint_state)
    {
      ...
      // rotYaw = goalYaw - vehicleYaw;
      // rotPitch = goalPitch - vehiclePitch;
      // if(abs(rotYaw)>0.1)
      // {
      //   if(rotYaw>0)
      //   {
      //   vehicleYawRate = 1.0;//0.5*rotYaw
      //   }
      //   else
      //   {
      //     vehicleYawRate = -1.0;//-0.5*rotYaw
