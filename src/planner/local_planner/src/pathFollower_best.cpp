...
int main(int argc, char** argv)
{
  ...
  cmd_vel.header.frame_id = "map_link";//"vehicle"
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      if (pubSkipCount < 0) {
        ...
        // vehicleSpeed_X2 = cos(vehicleYaw) * 1.0 + sin(vehicleYaw) * 0.0;
        // vehicleSpeed_Y2 = -sin(vehicleYaw) * 1.0 + cos(vehicleYaw) * 0.0;

        cmd_vel.twist.linear.x = vehicleSpeed_X2;
        cmd_vel.twist.linear.y = vehicleSpeed_Y2;
