...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      float vehicleXRel_m = vehicleX - vehicleXRec;
      float vehicleYRel_m = vehicleY - vehicleYRec;
      ...
      disX = mappathX - vehicleXRel_m;
      disY = mappathY - vehicleYRel_m;
      dis = sqrt(disX * disX + disY * disY);
      // std::cout<<"X: "<<disX<<endl;
      // std::cout<<"Y: "<<disY<<endl;
      float pathDir_x = atan2(disY, disX);
      float pathDir_y = atan2(disY, disX) - PI/2;
      printf("pathDir_x: %f , pathDir_y: %f \n",pathDir_x,pathDir_y);
      ...
      // printf("dirdiff_x: %f , dirdiff_y: %f \n",dirDiff_x,dirDiff_y);
      ...
      if(!navpoint_state)
      {
        ...
        //   {
        //   vehicleSpeed_Y = (vehicleSpeed_Y/fabs(vehicleSpeed_Y))*fabs(vehicleSpeed_X)*(fabs(disY)/fabs(disX));
        //   }
        // }
        ...
        //   {
        //   vehicleSpeed_X = (vehicleSpeed_X/fabs(vehicleSpeed_X))*fabs(vehicleSpeed_Y)*(fabs(disX)/fabs(disY));
        //   }
        // }


        //控制变量法测试
        vehicleSpeed_X = cos(vehicleYaw) * 1.0 + sin(vehicleYaw) * 0.0;
        vehicleSpeed_Y = -sin(vehicleYaw) * 1.0 + cos(vehicleYaw) * 0.0;

        cmd_vel.twist.linear.x = vehicleSpeed_X;
        cmd_vel.twist.linear.y = vehicleSpeed_Y;
