...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloud2) {
  ...
  for (int i = 0; i < laserCloudSize; i++) {
    ...
    if (pointZ > minRelZ - disRatioZ * dis &&
        pointZ < maxRelZ + disRatioZ * dis &&
        ...
        if (point.z - vehicleZ > minRelZ && point.z - vehicleZ < maxRelZ) {
          ...
          if (indX >= 0 && indX < planarVoxelWidth && indY >= 0 &&
            ...
            if (planarVoxelDyObs[planarVoxelWidth * indX + indY] <
                ...
                !clearDyObs) {
              // float disZ =
              //     point.z - planarVoxelElev[planarVoxelWidth * indX + indY];//计算点云相对高程信息

              float disZ =
                  point.z - (vehicleZ-0.0);//zbh
              // std::cout<<"minelev: "<<planarVoxelElev[planarVoxelWidth * indX + indY]<<endl;
              if (considerDrop)
                disZ = fabs(disZ);
