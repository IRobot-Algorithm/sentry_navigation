...
void EGOReplanFSM::goal_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    if (success)
    {
        ...
        else if (exec_state_ == ADJUST_POSE)
        {
            changeFSMExecState(GEN_NEW_TRAJ, "TRIG");
            is_target_receive=true;
        }    
  ...
  void EGOReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr &msg)
  {
    ...
    double norm = odom_vel_.head(2).norm();
    odom_vel_(0) = cos(yaw) * norm;
    odom_vel_(1) = sin(yaw) * norm;
    ...
    // {
    //   odom_vel_(0) = cos(yaw) * 0.03;
    //   odom_vel_(1) = sin(yaw) * 0.03;
    // }
#ifdef sim
    Eigen::Matrix2d R;
    R << std::cos(yaw),  std::sin(yaw),
        -std::sin(yaw),  std::cos(yaw);
    odom_vel_.head(2) = R.transpose() * odom_vel_.head(2);
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    if (exec_state_ == ADJUST_POSE && new_state != ADJUST_POSE)
    {
...
//        }`
    ...
    case EXEC_TRAJ:
    {
      ...
      // if (t_cur > info->duration_ - 1e-2 && (end_pt_.head(2) - odom_pos_.head(2)).norm() < 0.2)
      // {
      //   have_target_ = false;
      ...
      //   return;
      // }
      // else if ((start_pt_.head(2) - odom_pos_.head(2)).norm() < replan_thresh_)
      // {
      //   // cout << "near start" << endl;
      //   return;
      // }
      if (t_cur > info->duration_ - 1e-2 && (end_pt_ - pos).norm() < 0.5)
      {
        have_target_ = false;
  ...
  void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
  {
    ...
    // if (map->getInflateOccupancy2d((odom_pos_ + odom_vel_ * 0.3).head(2)))
    // {
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    std::cout << "start_pt_:" << std::endl << start_pt_ << std::endl;
    std::cout << "local_target_pt_:" << std::endl << local_target_pt_ << std::endl;
