...
Scan2MapLocation::Scan2MapLocation() : private_node_("~"), tf_listener_(tfBuffer_)
{
    ...
    //初始化订阅者
    laser_scan_subscriber_ = node_handle_.subscribe("/livox_pcl", 1, &Scan2MapLocation::ScanCallback, 
                                                      this,  ros::TransportHints().tcpNoDelay());

    map_subscriber_ = node_handle_.subscribe("pcl_output", 1, &Scan2MapLocation::MapCallback, this);

    odom_subscriber_ = node_handle_.subscribe("Odometry_baselink", 20, &Scan2MapLocation::OdomCallback,
                                                 this,  ros::TransportHints().tcpNoDelay());
    ...
    location_publisher_ = node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>("location_match", 1, this);//ICP匹配结果，进入EKF node
    ...
    relocate_initialpose_publisher_ = node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1, this);//重定位结果
    ...
    // 指针的初始化
    cloud_map_ = boost::shared_ptr<PointCloudT>(new PointCloudT());  //pcl::PointCloud<PointXYZ>
 ...
 * Map回调函数 进行数据处理
 */
void Scan2MapLocation::MapCallback(const sensor_msgs::PointCloud2::ConstPtr& map_msg)
{
    ...
    pcl::fromROSMsg(*map_msg, *cloud_map_);
...
void Scan2MapLocation::ScanCallback(const sensor_msgs::PointCloud2::ConstPtr &scan_msg)
{
    ...
    if(need_relocalization){
    ...
    else{   //正常Use_TfTree_Always位

        // step1 进行数据类型转换
        start_time_ = std::chrono::steady_clock::now();
        // ScanToPointCloudOnMap(scan_msg, cloud_scan_); // LaserScan -> pcl::PointCloudXYZ

        pcl::fromROSMsg(*scan_msg, *cloud_scan_); // sensor_mag::PointCloud2 -> pcl::PointCloudXYZ
        end_time_ = std::chrono::steady_clock::now();
        time_used_ = std::chrono::duration_cast<std::chrono::duration<double>>(end_time_ - start_time_);
        if (if_debug_){
        ...
        //使用ICP进行点云匹配
        match_result_ = Eigen::Isometry3d::Identity();//构造变换矩阵
...
*/
bool Scan2MapLocation::ReLocationWithICP(Eigen::Isometry3d &trans ,const sensor_msgs::PointCloud2::ConstPtr &scan_msg, PointCloudT::Ptr &cloud_map_msg, const Eigen::Isometry3d &robot_pose)
{
    //将雷达和地图点云保存为pcd文件
    if (save_pcd_){
        // 获取当前时间并格式化成字符串
        ...
        pcl::fromROSMsg(*scan_msg, *cloud_scan_);//zbh
 ...
 */
void Scan2MapLocation::ScanToPointCloudOnMap(const sensor_msgs::PointCloud2::ConstPtr &scan_msg, PointCloudT::Ptr &cloud_msg)
{
    pcl::PointCloud<pcl::PointXYZI>::Ptr livoxcloud_msg(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::fromROSMsg(*scan_msg, *livoxcloud_msg);
    // 对容器进行初始化
    unsigned int point_num = 0;
    unsigned int point_len = 0;
    point_len = livoxcloud_msg->points.size();
    cloud_msg->points.resize(point_len);

    tran_start_time_ = std::chrono::steady_clock::now();
    for (unsigned int i = 0; i < livoxcloud_msg->points.size(); ++i)
    {
            // 首先声明一个 cloud_msg第i个点的 引用
            PointT &point_tmp = cloud_msg->points[point_num];

            Eigen::Vector3d point_vector(livoxcloud_msg->points[i].x , livoxcloud_msg->points[i].y , livoxcloud_msg->points[i].z);
